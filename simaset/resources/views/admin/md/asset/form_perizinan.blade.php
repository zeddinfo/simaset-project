<tr data-index-perizinan="{{$i}}">
    ...
    <td>
    <input type="text" id="{{$name}}_{{$i}}_nomor" class="form-control numeric" name="{{$name}}[{{$i}}][nomor]" placeholder="" value="{{isset($model) ? $model->nomor : ''}}"  required="" data-id="nomor" im-insert="true">
    </td>
    <td>
    <input type="text" id="{{$name}}_{{$i}}_tgl_izin" class="form-control" name="{{$name}}[{{$i}}][tgl_izin]" placeholder="" data-date-format="dd/mm/yyyy" value="{{isset($model) ? date('d-m-Y', strtotime($model->tgl_izin)) : date('Y-m-d')}}" required="" data-id="tgl-izin" im-insert="true">
    </td>
</tr>
